...
### Requirement: SQLite storage keyed by manufacturer and URL

The provider SHALL persist responses in a SQLite database. Each stored response SHALL be a row keyed by the pair `(manufacturer, url)`, holding at least the raw response text, the fetch timestamp, and a status flag distinguishing a successful fetch from a failed one. Writing a fresh successful response for an existing `(manufacturer, url)` SHALL replace that row's content and timestamp. Distinct URLs for the same manufacturer SHALL be stored as distinct rows and SHALL NOT overwrite one another. For non-GET requests (e.g. Microchip's MCP `POST`s), the stored key SHALL incorporate a stable hash of the request method and body so that different calls to the same endpoint do not collide. The database SHALL be created if it does not exist.

#### Scenario: Distinct URLs coexist for one manufacturer

- **WHEN** two different URLs are stored for the same manufacturer
- **THEN** both rows exist independently
- **AND** reading either URL returns its own content

#### Scenario: Re-storing a URL replaces its content and timestamp

- **WHEN** a URL already has a stored row and a new successful response is stored for it
- **THEN** the row's content and fetch timestamp reflect the new response
...
The system SHALL load cache configuration — at least the cache database path, the TTL, and an enable/disable flag — from an optional `config.yaml`, falling back to committed defaults when the file is absent. When the cache is disabled by configuration, `fetch` SHALL behave as a direct pass-through to a live fetch in every mode (no storage read or write), preserving today's behavior.
