...
- [ ] 2.8 Staleness helper (age vs `ttl_days`); module-level provider singleton + `configure(config)` used by the CLI; adapters call the module-level `fetch`
...
- [ ] 4.1 Configure the provider (cache-first) for the interactive search path
