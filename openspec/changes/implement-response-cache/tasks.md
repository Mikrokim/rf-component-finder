...
- [ ] 1.1 Implement `load_cache_config()` returning `cache_db` path, `ttl_days` (default 7), and `enabled` (default true); read from `config.yaml` if present, else committed defaults
...
- [ ] 1.3 Commit `config.example.yaml` documenting `cache_db` / `ttl_days` / `enabled`
...
- [ ] 2.1 Create SQLite schema: one table keyed `(manufacturer, key)` with columns `content`, `fetched_at`, `status`; create DB/table if missing; enable WAL, set `busy_timeout`, open with `check_same_thread=False`
- [ ] 2.2 Implement key derivation: plain `url` for GET; `url + "#" + stable_hash(method, params, json)` for non-GET / bodied requests
...
- [ ] 2.5 `refresh` path: live fetch → store fresh + return on success; on failure keep the prior row (timestamp unchanged) and return it (serve-stale); `None` when failed and no prior row
...
- [ ] 2.7 Serialize DB writes behind a `threading.Lock` so Microchip's refresh-time thread pool is safe (free: refresh is not latency-sensitive)
...
- [ ] 3.2 Non-GET keys: same URL with different bodies stored separately
