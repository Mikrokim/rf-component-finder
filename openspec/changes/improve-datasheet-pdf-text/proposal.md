...
- **Pipe-table page text.** Render each detected table as an explicitly-delimited pipe table (per-cell bounding boxes), with prose interleaved, so a number's column (Min/Typ/Max) is *marked*, not merely implied by position — this is what fixed field placement in measurement (Drain Voltage `{"value":"28"}` → `{"typ":28}`). A per-page content-preservation guard falls back to `layout=True` when the pipe rendering would drop any token, so the safe behaviour is never lost.
- **De-duplicated page furniture, guarded.** A line repeating on ~every page is furniture: keep its **first** occurrence, drop the repeats. A narrow table-header guard (`min|max|typ|nominal|parameter|units|symbol|conditions|rating|value`) is never de-duplicated, so a spec-table header can never be collapsed and cost us the column context the pipe rendering exists to mark. Verified against all five datasheets: the guard matches **none** of the real furniture (so it costs no savings) and **all** of the real table headers (so it protects every one).
...
- `datasheet-extraction`: the **Extract datasheet text from a PDF** requirement changes. It currently specifies plain `extract_text()` joined by blank lines. It gains pipe-table rendering with per-cell text and a layout-mode fallback, guarded furniture de-duplication, and a new segmentation entry point that yields relevance-ordered, budget-sized text. The LLM extraction-contract and `RawValue`-mapping requirements in the same capability are untouched.

## Impact

- **Code:** `rf_finder/datasheet/pdf.py` only. `extractor.py`, `mapping.py`, and `config.py` are untouched.
- **Tests:** `tests/test_datasheet_pdf.py`. The **pure** logic — pipe-formatting, the empty-table filter, header synthesis, the content-preservation check, furniture de-duplication, and segmentation — MUST stay unit-testable with fake grids/lines/token-sets, no real PDF, preserving `_join_page_text`'s dependency-free testability. The **geometry-dependent** layer — `find_tables`, per-cell `crop`, and word-in-cell bbox filtering — cannot be faked and SHALL be covered by integration tests over the `evals/pdfs/` datasheets. Implementation SHOULD separate the two so the pure part carries the bulk of the coverage.
- **Dependencies:** none added. `find_tables`, per-cell `crop`, `extract_words` and `layout=True` are all existing `pdfplumber` features; the filter, header synthesis, guard, de-duplication and scoring are plain Python. (`pymupdf4llm` was evaluated and rejected: it is AGPL-3.0, and the free path already recovers the measured benefit.)
