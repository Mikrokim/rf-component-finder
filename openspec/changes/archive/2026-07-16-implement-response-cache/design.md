...
The `design.md` (§9.1) and the `cache.py`/`config.py` stubs sketch a response cache keyed by `(adapter, normalized_query_url)` with a TTL. This change implements it as a central **HTTP service** that owns all network access, composed over a separate **storage layer** (see D3); adapters ask the service for a page by URL, and it is stored as a plain file in a local directory (see D2). The service is **cache-first**: a search serves from the local cache and only reaches the network on a miss or when a copy is older than the TTL (**30 days**). There is no scheduled/background auto-refresh — the cache updates only as a side effect of a user request (or a manual `refresh`).

Relevant current facts the design must respect:
- Per-manufacturer politeness delays are already hard-coded per adapter as `_MIN_DELAY_SECONDS` and they matter (fetching too fast gets the client blocked/disconnected): Mini-Circuits / 3rWave / RWM = 1 s, AmcomUSA / Marki = 1.5 s, Qorvo / Guerrilla RF / VectraWave = 2 s, UMS = 3 s, Analog Devices = 5 s, MACOM = 60 s. Existing per-adapter fetch timeouts are 30–60 s. The HTTP service inherits these exact values, so site-facing behavior is unchanged and a slow-but-valid site (Qorvo's 5.3 MB page) is given the time it needs rather than cut off early.
...
**Goals:**
- A single **HTTP service** that all adapters retrieve through; no adapter opens a socket directly. The service owns all network access and composes a separate storage layer for caching (see D3).
...
- Centralize User-Agent / per-manufacturer delay / retries in the HTTP service; remove that duplicated boilerplate from adapters.
...
### D3 — Two layers: an HTTP **service** over a storage **mechanism**, reached via a module-level `http.fetch()`
Retrieval is split into two single-responsibility layers:
- **`rf_finder.http`** — the HTTP **service**. It owns *all* network access (httpx, the shared User-Agent, per-manufacturer delay, per-site timeout, retries, TLS `verify`, the concurrency lock) and the cache-first policy (fresh-serve, expiry, stale-fallback, background revalidate). It is the adapter-facing entry point.
- **`rf_finder.cache`** — the storage **mechanism**. It only derives file paths, reports freshness (file `mtime` vs TTL), and reads/writes atomically. It never touches the network. The service composes one of these.

Adapters call `rf_finder.http.fetch(manufacturer, url, **opts)` in place of `httpx.get(...)`; they are oblivious to whether a page came from cache or the network. A single service instance is configured once per process (by the CLI) with the cache directory and config.

**Alternatives:** (a) one combined `ResponseCache` class that both fetches and stores — fewer files, but it mixes two responsibilities (network mechanics and cache storage), which makes each harder to test and change in isolation; (b) inject the service into `search(spec, service)` — explicit but churns all 12 signatures and their tests; (c) set an attribute on each adapter before calling — implicit/ugly. **Why the split + module-level helper:** separating the HTTP concern from the cache concern means each layer is tested on its own (storage with no sockets; the service with a stubbed transport) and caching can be disabled/replaced without touching adapters; the module-level helper keeps the only per-adapter change at the fetch call site (`search(spec)` signature preserved). Trade-off: a process-global service; acceptable for a single-run CLI, and it is the natural singleton.
...
1. Land the HTTP service (`http`) + storage layer (`cache`) + `config` (cache-first `fetch`, TTL 30 days, `enabled=true`); no adapter changes yet.
2. Migrate adapters one at a time (`httpx.get` → `http.fetch`), starting with **Qorvo** (the motivating failure), validating each against its saved fixture so parsing is provably unchanged.
3. Wire `__main__` search to the cache-first HTTP service; join outstanding background revalidations (bounded) after results are displayed, before exit.
