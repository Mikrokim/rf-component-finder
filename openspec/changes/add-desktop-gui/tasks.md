...
- [x] 1.2 Refactor `run_search` in `rf_finder/__main__.py` to call `search_and_verify` (passing an `on_source` callback that prints the existing per-source lines with `cli._snapshot_note`) while keeping all its `print` output verbatim; drop the now-unused `_load_adapters`/`verify` locals
- [x] 1.3 Repoint `cli.py`'s import to `from rf_finder.search import _load_adapters` (and update `tests/test_main.py` to patch `_load_adapters` at its new home)
- [x] 1.4 Run the existing test suite to confirm the refactor is behavior-preserving — 427 passed
...
- [x] 2.3 In `main()`, configure the shared cache exactly as the CLI does: `cache.configure(load_cache_config())`, then build and run the Tk root window
...
- [x] 9.3 Confirm `python -m rf_finder` (the existing CLI) still runs unchanged — `--help` OK; `tests/test_main.py` green
