...
- [ ] 3.5 Contain per-candidate enrichment failures (`DatasheetFetchError`, extraction/LLM errors) so the run continues; keep `requests`/LLM imports lazy
- [ ] 3.6 Gate 2: re-`verify()` enriched candidates; return only those with `overall == "match"`
...
- [ ] 6.5 A raising adapter / failing datasheet does not abort the run
