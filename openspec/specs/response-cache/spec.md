...
Define the shared HTTP service and its filesystem storage layer that mediate all
adapter network access. Adapters retrieve every source document through this
service, which resolves requests cache-first, owns rate-limiting/retries, and
persists responses so repeated runs avoid needless network traffic. This spec
documents the caching behavior, its cache-first resolution model (fresh /
expired / missing), background revalidation, the manual `refresh` command, and
the cache-scoped configuration.
