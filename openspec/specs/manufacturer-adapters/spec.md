...
Adapters SHALL NOT open network connections directly. Every source document an adapter retrieves SHALL be obtained through the shared HTTP service (`http.fetch(manufacturer, url, ...)`). The per-adapter browser User-Agent, minimum inter-request delay, and transient-failure retry behavior SHALL be supplied by the HTTP service rather than implemented in the adapter; each adapter's target URLs, HTML/JSON parsing, and ontology mapping SHALL be unchanged by this. When the HTTP service returns no document for a URL (no cached copy and a failed live fetch), the adapter SHALL treat that source page as unavailable and skip it rather than raise.
...
#### Scenario: Adapter retrieves through the HTTP service, not the network

- **WHEN** an adapter's `search` runs and needs a source page
- **THEN** it obtains the page via the HTTP service's `fetch`
- **AND** it does not issue a direct `httpx` network call of its own

#### Scenario: Parsing is unchanged when served from the HTTP service

- **WHEN** an adapter parses a page served by the HTTP service from a saved fixture
