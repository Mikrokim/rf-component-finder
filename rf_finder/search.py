...
def search_and_verify(spec, *, on_source=None):
    ...
    ``provider`` is intentionally not a parameter: adapters reach the network
    through the process-global provider installed by ``cache.configure`` — the
    caller must have configured it before calling this.
