...
class App:
    ...
    def __init__(self, root: tk.Tk, provider, max_results: int = DEFAULT_MAX_RESULTS) -> None:
        self.root = root
    ...
    def _worker(self, spec) -> None:
        ...
        # Heal the cache in the background (bounded), like the CLI — no print here.
        try:
            self.provider.join_revalidations()
        except Exception:
            pass
...
def main() -> None:
    """Configure the shared cache (exactly as the CLI does) and run the window."""
    ...
    App(root, provider, max_results=load_max_results())
    root.mainloop()
