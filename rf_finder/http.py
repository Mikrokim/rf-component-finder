"""Central HTTP service: every adapter fetches through it (NFR-1, NFR-2, design.md D3/D4).

This is the single service responsible for **all** outbound HTTP. Adapters call
the module-level :func:`fetch` in place of ``httpx`` directly; the service owns
the shared User-Agent, the per-manufacturer politeness delay, per-site timeouts,
transient-failure retries, and TLS options — boilerplate that used to be copied
into each of the twelve adapters.

Caching is a **mechanism used inside this service**, not something the adapters
see. The service is cache-first: it asks its :class:`~rf_finder.cache.ResponseCache`
for a fresh copy and serves it with no network; only on a miss / expiry does it
hit the network, then stores the result. On an expired page whose live re-fetch
fails it serves the stale copy and heals the cache on a background thread; a
missing page that can't be fetched yields a result whose ``text`` is ``None`` so
the caller skips that source.
"""
...
# ---------------------------------------------------------------------------
# Constants (seeded from the per-adapter values this service now owns)
# ---------------------------------------------------------------------------
...
class FetchResult:
    """The outcome of a :meth:`HttpService.fetch` call.
...
class HttpService:
    """Cache-first HTTP service shared by every adapter (one instance per process)."""

    def __init__(self, config: CacheConfig) -> None:
        self._config = config
        self._cache = cache.ResponseCache(config.cache_dir, config.ttl_days)
    ...
    def fetch(
        ...
        *,
        ...
        path = self._cache.path(
            manufacturer, url, method=method, params=params, json_body=json,
        )
        age = self._cache.age_seconds(path)
        fresh = age is not None and age <= self._cache.ttl_seconds

        # Fresh: serve from disk untouched (unless refresh mode forces a re-fetch).
        if fresh and not self._refresh_mode:
            result = FetchResult(
                text=self._cache.read(path), age_seconds=age, from_cache=True,
            )
        ...
        if age is not None:
            ...
            result = FetchResult(
                text=self._cache.read(path), age_seconds=age,
                served_stale=True, from_cache=True,
            )
            self._record_served(manufacturer, result)
            return result
        ...
        *,
        ...
        with self._revalidate_guard:
            ...
            thread = threading.Thread(
                target=_run, name=f"revalidate:{manufacturer}", daemon=True
            )
...
_service: HttpService | None = None


def configure(config: CacheConfig) -> HttpService:
    """Create (or replace) the process-wide HTTP service from ``config``; return it.

    The CLI calls this once at startup. Tests call it with a ``tmp_path``-based
    config to get an isolated cache.
    """
    global _service
    _service = HttpService(config)
    return _service


def fetch(manufacturer: str, url: str, **kwargs) -> FetchResult:
    """Adapter-facing entry point: fetch ``url`` through the configured service.

    Raises ``RuntimeError`` if :func:`configure` has not been called yet — the
    service must be set up before any adapter runs.
    """
    if _service is None:
        raise RuntimeError("HTTP service not configured; call http.configure(config) first")
    return _service.fetch(manufacturer, url, **kwargs)
...
def join_revalidations(max_wait: float = _REVALIDATE_JOIN_SECONDS) -> None:
    ...
    if _service is not None:
        _service.join_revalidations(max_wait)
