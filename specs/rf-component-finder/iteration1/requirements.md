...
- **REQ-1.5** — For every parameter with a numeric range or bound — the band params `contains` (e.g. `freq_range`, `Temperature`, `VDD`) **and** the bounded-scalar params `between`/`min`/`max` (e.g. `P1dB`, `Gain`, `NF`, `Psat`, `length`, `width`, `MSL`) — the system SHALL provide separate `min` and `max` input fields. For every one of these except `contains`, either side may be omitted: an omitted `min` defaults to `-∞` and an omitted `max` defaults to `+∞` (a one-sided range — an omitted bound imposes no restriction). Filling only the natural side of a `min`/`max` param reproduces its namesake behavior (e.g. `Gain` min-only → "≥ x"; `NF` max-only → "≤ x"), while the extra side lets the user cap or bracket the value (e.g. `Gain` between 20 and 30 dB). Such a bounded-scalar query is recorded as a (one-sided) `between` constraint.
...
- **REQ-2.2** — The system SHALL support at least the following parameters for amplifiers: frequency range (`freq_range`), P1dB, Gain, Noise Figure (NF), IP3, Psat, VDD (supply voltage), physical length and width (`length`, `width`), MSL (moisture sensitivity level, 1–5), and operating Temperature.
...
- **REQ-2.4** — The system SHALL define a comparison rule per parameter: `freq_range`=`contains`, `P1dB`/`Gain`/`IP3`/`Psat`=`min` (candidate should be at least the required value), `NF`/`length`/`width`/`MSL`=`max` (candidate should be at most the required value), `VDD`=`contains` (the part's supply-voltage range must contain the requested voltage), `Temperature`=`contains` (the part's operating temperature range must contain the required range). The `min`/`max` rule denotes the parameter's **primary bound direction**; per REQ-1.5 the user may still supply the opposite bound, in which case the query becomes a two-sided `between` (candidate value must fall within `[min, max]`).
- **REQ-2.5** — The system SHALL convert between equivalent units (MHz↔GHz, dBm↔W, mW↔dBm, and length units cm/inch/mil→mm for length/width), and SHALL treat dimensionless ratio units (dB, for Gain/NF) as an identity conversion (dB→dB), through a dedicated conversion module.
