...
- **REQ-1.5** — For range parameters — both `contains` (e.g. `freq_range`) and bounded-scalar `between` (e.g. `P1dB`/`Gain`/`OIP3`) — the system SHALL provide separate `min` and `max` input fields. For a `between` parameter either side may be omitted: an omitted `min` defaults to `-∞` and an omitted `max` defaults to `+∞` (a one-sided range — an omitted bound imposes no restriction).
...
- **REQ-2.4** — The system SHALL define a comparison rule per parameter: `freq_range`=`contains`, `P1dB`/`Gain`/`OIP3`=`between` (candidate value must fall within a min/max band), `Pout`=`min`, `NF`=`max`.
